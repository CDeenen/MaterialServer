//Sets the default port of the websocket
let port = 3001;
...
for (let i=0; i<myArgs.length; i++){
    let arg = parseInt(myArgs[i]);
    if (isNaN(arg) == false) port = arg;
    else if (myArgs[i] == 'debug') {
        DEBUG = true;
        console.log('Debugging enabled');
    }
    else DEBUG = false;
}
console.log("Starting Material Server "+version);
